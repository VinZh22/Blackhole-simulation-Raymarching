...
//
////// Ray marching shader
// Implements Basic cloud creation.
...
vec3 cloud_1(vec3 p) { // A Spherical cloud
  float r = length(p-vec3(-5,0,0));
  return vec3(-0.5/(1+10*r*r));
}
...
vec3 cloud_2(vec3 p) { // a torus shaped cloud using an  sdf
  p = p*vec3(1,3,1);
  float d = sdfTorus(p, vec2(2,1), vec3(5,0,0), ID3);
  return -0.1*vec3(max(-d,0));
}

// We use a vec4 to store information on color and depth
vec4 rayMarch(	vec3 ro, 		// ray origin
  ...
  for (int i = 0; i < MAX_MARCHING_STEPS; i++) {
    vec3 p = ro + depth * rd;
    float d = sdSphere(p, SPHERE_CENTER, 0.1);
    float campled_d = min(d,MAX_DX);
    depth += campled_d;
    col += max(campled_d,0) * ( cloud_1(p) + cloud_2(p) );
...
void main() {
  ...
  vec4 col_depth = rayMarch(ro, rd, MIN_DIST, MAX_DIST); // distance to sphere

  vec3 cloud_col = col_depth.xyz;
  float depth = col_depth.w;

  vec3 pos = ro + rd*depth;

  float d = sdSphere(pos, SPHERE_CENTER, 1);
  ...
  if (d > 2*PRECISION) {
    ...
    col = rand_stars+galaxy_bg-cloud_col;
